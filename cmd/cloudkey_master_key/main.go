...
func main() {
	...
	//var sessions []*eidc32proxy.Session

	whiteLed, err := cloudkeyled.New(whiteDir)
	if err != nil {
		log.Fatal(err)
	}
	whiteLed <- cloudkeyled.Command{Off: true}

	blueLed, err := cloudkeyled.New(blueDir)
	if err != nil {
		log.Fatal(err)
	}
	blueLed <- cloudkeyled.Command{Off: true}
...
MAINLOOP:
	for {
		...
		case s := <-sessChan: // new session has come up
			...
			blueLed <- cloudkeyled.Command{
				Pattern: []int{len(sessions)},
				Repeat: true,
			}
			//digits, err := utils.IntToBaseTenDigits(len(sessions))
			//if err != nil {
			//	log.Println(err)
			//}
			//blueLed <- cloudkeyled.LedSetting{Pattern: digits}
